...
Distributed job orchestration prototype: **Python control plane** (API, scheduling, Postgres state) + **Go workers** (Kafka consume/execute/publish) + **Kafka** between the planes. **Day 112:** worker **execution idempotency** integrated — `execution:<job_id>:<attempt>`, `event=duplicate_worker_execution`, backends `disabled|memory|redis` (default **disabled**) — **[docs/design/worker-execution-idempotency.md](docs/design/worker-execution-idempotency.md)**.
...
| [docs/design/worker-execution-idempotency.md](docs/design/worker-execution-idempotency.md) | Worker execution dedupe integrated (Day 112) |
