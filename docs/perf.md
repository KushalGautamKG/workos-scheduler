...
| `work_queue_capacity` | Configured bounded work-queue size for this run | Baseline for saturation—compare to **`work_items_enqueued`** and **`work_queue_full_errors`** |
| `work_items_enqueued` | Decoded jobs successfully placed on the worker pool queue | Healthy handoff from Kafka poll loop to executors |
...
**Bounded work queue (worker plane):** **`cmd/consumer`** uses a **bounded in-process queue** between the Kafka poll loop and the worker pool (**default capacity 100**, env **`KERNELQ_WORKER_QUEUE_CAPACITY`**). When full, **`Enqueue`** returns **`worker queue full`**, logs **`event=worker_queue_full`**, and **`work_queue_full_errors`** increments—KernelQ’s **first backpressure boundary** (**visibility**, not full backpressure yet). Shutdown logs **`work_queue_capacity`**, **`work_items_enqueued`**, and **`work_queue_full_errors`**. **Day 81** adds a **deterministic queue saturation test** (`TestWorkerQueueSaturationStats`, **`./worker/scripts/smoke_queue_saturation.sh`**) that validates queue-full stats and logging—**Kafka pause/resume** and **worker autoscaling** remain **future work**.
