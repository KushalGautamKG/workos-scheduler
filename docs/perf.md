...
**Bounded work queue (worker plane):** **`cmd/consumer`** uses a **bounded in-process queue** between the Kafka poll loop and the worker pool (**default capacity 100**, env **`KERNELQ_WORKER_QUEUE_CAPACITY`**). When full, **`Enqueue`** logs **`event=worker_queue_full`**, increments **`work_queue_full_errors`**, and applies **local backoff** (**Day 82**: **50ms** sleep, **one retry**) to **reduce enqueue pressure during bursts**—KernelQ’s **first backpressure boundary** (**visibility + light throttle**, not full backpressure). **Day 81** adds a **deterministic saturation test** (`./worker/scripts/smoke_queue_saturation.sh`). **Kafka pause/resume** (design only, no implementation yet) is documented in [`docs/design/kafka-pause-resume-backpressure.md`](design/kafka-pause-resume-backpressure.md): **high/low watermarks** pause partition fetch when saturated and resume after drain; future metrics include **`kafka_pause_events`**, **`kafka_resume_events`**, and **`worker_queue_depth`**. **Worker autoscaling** remains separate future work.
