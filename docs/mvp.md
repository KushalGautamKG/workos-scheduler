...
- **Local Redis service** — `docker compose up -d redis`; **Day 101** result idempotency smoke (`smoke_result_idempotency_redis.py`; redis-cli, no Python Redis package)
...
- **Redis dedupe** — **Day 100** consumer dedupe; **Day 101** Redis smoke for **`worker_result_key`** + **`SET NX EX`**; full Kafka replay smoke still future; dispatch/execution still future — **[redis-idempotency-deduplication.md](design/redis-idempotency-deduplication.md)**
