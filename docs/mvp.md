...
- **Local Redis service** — `docker compose up -d redis` (`kernelq-redis`, `:6379`) for planned idempotency/dedupe; design **[redis-idempotency-deduplication.md](design/redis-idempotency-deduplication.md)** — **no app logic wired yet**
...
- **Redis dedupe** — local **`redis`** service in Docker Compose; idempotency design in **[redis-idempotency-deduplication.md](design/redis-idempotency-deduplication.md)** — **not integrated in control plane or workers yet**
