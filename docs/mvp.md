...
- **Local Redis service** — `docker compose up -d redis` (`kernelq-redis`, `:6379`); **Day 97** **`IdempotencyStore`** + test-only **`InMemoryIdempotencyStore`** — starts duplicate suppression after Day 96 design foundation
...
- **Redis dedupe** — **`IdempotencyStore`** boundary exists; in-memory fake is **test-only**; **Redis-backed store** and worker/result wiring **future work** — **[redis-idempotency-deduplication.md](design/redis-idempotency-deduplication.md)**
